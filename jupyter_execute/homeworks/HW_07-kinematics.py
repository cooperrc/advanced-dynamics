...
for i,tt in enumerate(t):
    q[:,i]=fsolve(lambda q: C_slidercrank(q,tt),q0,                    fprime= lambda q: Cq_slidercrank(q,tt))
    q0=q[:,i]
...
def vel_acc(q,t):
    ...
    Qd[5:7]=np.reshape(dq[5]**2*rotA(q[5])@np.array([[l2/2],[0]])-        dq[8]**2*rotA(q[8])@np.array([[-l3/2],[0]]),2)
    Qd[7:9]=np.reshape(dq[8]**2*rotA(q[8])@np.array([[l3/2],[0]])-        dq[11]**2*rotA(q[11])@np.array([[0],[0]]),2)
